...
for the MycoDiscovery systematic search.
...
Design goals (all driven by "this needs to survive peer review"):
  ...
  4. NEVER auto-merges results into the trusted database. This module only
     ...
     candidate before it's promoted into mycodiscovery_targets_v1.json — see
...
class PubMedClient:
    ...
    def search_with_count(self, query: str, protocol_version: str, retmax: int = 1000,
                           use_cache: bool = True):
        """Like search(), but also returns PubMed's reported TOTAL hit count
        (result['Count']), which can be far larger than retmax. Intended for
        broad, gene-unrestricted queries where you need to know how much was
        left un-fetched, not just what you got back. Returns (pmids, total_count)."""
        cache_file = self._cache_key("esearch_count:" + query)
        if use_cache and cache_file.exists():
            data = json.loads(cache_file.read_text())
            pmids, total = data["pmids"], data["total"]
            self._log({
                "stage": "esearch_with_count", "query": query, "protocol_version": protocol_version,
                "n_fetched": len(pmids), "total_available": total, "cache_hit": True,
            })
            return pmids, total
        ...
        cache_file.write_text(json.dumps({"query": query, "pmids": pmids, "total": total}))
        self._log({
            "stage": "esearch_with_count", "query": query, "protocol_version": protocol_version,
            "n_fetched": len(pmids), "total_available": total, "cache_hit": False,
        })
        return pmids, total
